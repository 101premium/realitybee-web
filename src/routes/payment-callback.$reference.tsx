...
export const Route = createFileRoute("/payment-callback/$reference")({
  ...
  head: () => ({
    meta: [{ title: "Confirming payment — SolunaSoul" }, { name: "robots", content: "noindex" }],
  }),
...
function PaymentCallbackPage() {
  ...
  const { adviceReference, paymentReference, status } = Route.useSearch();
      ...
      {!verifying && (
        <dl className="mx-auto mt-8 max-w-sm space-y-2 rounded-md border border-border bg-card p-4 text-left text-sm">
          <div className="flex justify-between gap-4">
            ...
            <dd className="font-mono">{adviceReference ?? reference}</dd>
          </div>
          {(paymentReference ?? result?.paymentReference) && (
            <div className="flex justify-between gap-4">
              <dt className="text-muted-foreground">Payment reference</dt>
              <dd className="font-mono">{paymentReference ?? result?.paymentReference}</dd>
            </div>
          )}
        </dl>
      )}
